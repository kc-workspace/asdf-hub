...
## execute input command only if on debug mode
## e.g. `asdf_on_debug echo 'hello debugger'`
asdf_on_debug() {
  if [ -z "${DEBUG:-}" ]; then
    return 0
  fi
...
asdf_download() {
  ...
  local download
  download="https://github.com/GrantBirki/hub/releases/download/v${version}/hub-${os}-${arch}-${version}.tgz"
  asdf_info "starting [%15s] %s" \
    "download" "$download"

  local tmpfile
  tmpfile="hub-${os}-${arch}-${version}.tgz"
  local tmpdir
  tmpdir="$(mktemp -d)"
